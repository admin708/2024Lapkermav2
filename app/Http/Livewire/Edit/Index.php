...
class Index extends Component
{
    ...
    public function mount($id,$val)
    {
        ...
        if ($val == 'MoA') {
            $this->findDokumen = DataMoaDokumen::where('kerjasama_id',$id)->get();
            $findMe = DataMoa::find($id);
            $findMeTo = DataMoaPenggiat::where('id_lapkerma',$id);
            $findKegiatan = DataMoaBentukKegiatanKerjasama::where('id_moa',$id)->get();
            $this->jenis_dokumen_kerjasama = 2;
        } else {
            $this->findDokumen = DataIaDokumen::where('kerjasama_id',$id)->get();
            $findMe = DataIa::find($id);
            $findMeTo = DataIaPenggiat::where('id_lapkerma',$id);
            $findKegiatan = DataIaBentukKegiatanKerjasama::where('id_ia',$id)->get();
...
// dd(auth()->user());
        ...
        $this->sdgs = $findKegiatan->first()->id_sdgs;
    ...
    {
        $hapus = $this->findDokumen->find($id);
        $hapus->delete();
    }

    public function yakinHapusDokumen($id)
    {
        $this->yakinHapus = $id;
    }
    ...
    public function render()
    {
        ...
        if ($this->valz == 'MoA') {
            $this->findDokumen = DataMoaDokumen::where('kerjasama_id',$this->idEdit)->get();
        } else {
            $this->findDokumen = DataIaDokumen::where('kerjasama_id',$this->idEdit)->get();
        }

        $this->getProdiMitras = ProdiMitra::get();
        $this->getFakultasMitras = FakultasPihak::get();
        return view('livewire.edit.index');
    }
    ...
    {
        $this->validasiSave();
        $getAll = DataMoa::where('nomor_dok_unhas', $this->nomor_unhas)->get();
        // $arrayNamaProdiUH = [];
        // $arrayNamaPenggiat = [];
        $hitung=0;
        foreach ($this->inputs as $key => $value) {
            ...
            if ($namanama == 'universitas hasanuddin')
            {
                ...
                $namaPihak1 = $this->nama_pihak[$key]; $namaPejabat1 = $this->nama_pejabat_pihak[$key];
            ...
            try {
                    ...
                    if (!empty($array_not_in_prodi)) {
                        foreach ($array_not_in_prodi as $key => $value) {
                            $delete = DataMoa::where('nomor_dok_unhas', $this->nomor_unhas)->where('prodi_id', $value)->first();
                            $deletePenggiatKerjasama = DataMoaPenggiat::where('id_lapkerma', $delete->id)->delete();
                            $deleteBentukKegiatanKerjasama = DataMoaBentukKegiatanKerjasama::where('id_moa', $delete->id)->delete();
                            $delete->delete();
                        }
                    }
            
                    $getUpdate = DataMoa::where('nomor_dok_unhas', $this->nomor_unhas)->get();

                    // $find = DataMoa::find($id);
                    foreach ($getUpdate as $key => $find) {
                        ...
                        $findMeTo = DataMoaPenggiat::where('id_lapkerma',$find->id)->delete();
                            ...
                            foreach ($this->inputs as $key => $value) {
                                ...
                                $storePenggiatKerjasama = DataMoaPenggiat::create([
                                    'id_lapkerma' => $find->id, 'pihak' => $value+1, 'status_pihak' => $this->status[$key],
                        ...
                        $findMeTo = DataMoaBentukKegiatanKerjasama::where('id_moa',$find->id)->delete();
    ...
    {
        $this->validasiSave();
        $getAll = DataIa::where('nomor_dok_unhas', $this->nomor_unhas)->get();
        $arrayNamaProdiUH = [];
        // $arrayNamaPenggiat = [];
        $hitung=0;
        foreach ($this->inputs as $key => $value) {
            ...
            {
                $arrayNamaProdiUH = $this->arrayNamaProdi[$key];
                // array_push($arrayNamaProdiUH, $this->arrayNamaProdi[$key]);
                $status = $this->status[$key]; $alamatPihak1 = $this->alamat_pihak[$key];
                $namaPihak1 = $this->nama_pihak[$key]; $namaPejabat1 = $this->nama_pejabat_pihak[$key];
                $jabatanPejabat1 = $this->jabatan_pejabat_pihak[$key]??null; $pj1 = $this->pj_pihak[$key]??null;
                $jabatanPj1 = $this->jabatan_pj_pihak[$key]??null; $emailPj1 = $this->email_pj_pihak[$key]??null;
        ...
        if ($hitung == 0) {
            ...
            try {
                ...
                if (!empty($array_not_in_prodi)) {
                    foreach ($array_not_in_prodi as $key => $value) {
                        $delete = DataIa::where('nomor_dok_unhas', $this->nomor_unhas)->where('prodi_id', $value)->first();
                        $deletePenggiatKerjasama = DataIaPenggiat::where('id_lapkerma', $delete->id)->delete();
                        $deleteBentukKegiatanKerjasama = DataIaBentukKegiatanKerjasama::where('id_ia', $delete->id)->delete();
                        $delete->delete();
                    }
                }

                $getUpdate = DataIa::where('nomor_dok_unhas', $this->nomor_unhas)->get();
                ...
                foreach ($getUpdate as $key => $find) {
                    ...
                    $findMeTo = DataIaPenggiat::where('id_lapkerma',$find->id)->delete();
                    ...
                    $findMeTo = DataIaBentukKegiatanKerjasama::where('id_ia',$find->id)->delete();
